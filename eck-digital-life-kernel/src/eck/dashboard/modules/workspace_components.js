export const escapeHtml = (value) => String(value ?? "")
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#039;");
export const concise = (value, limit = 180) => {
...
const artifactMarkup = (items) => items.map((item) => `
...
export class HomeComponent {
    ...
    renderActivity(activity, resources) {
        const state = document.querySelector("#activity-status");
        if (state) {
        ...
        const progress = document.querySelector("#activity-progress-bar");
        ...
        const feed = document.querySelector("#activity-summary");
        if (feed) {
    ...
    renderProjects(id, projects, empty, result = false) {
...
export class LibraryComponent {
    render(data, append = false) {
      ...
      `;
        ...
        const more = document.querySelector("#load-more-library");
        if (more)
...
export class SkillComponent {
    render(data, append = false) {
        ...
        const target = document.querySelector("#skill-cards");
        if (!target)
            return;
    ...
    `).join("");
        ...
        const more = document.querySelector("#load-more-skills");
        if (more)
            more.hidden = data.page.next_offset === null;
    }
    text(id, value) {
...
const projectCard = (project, result = false) => `
