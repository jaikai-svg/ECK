...
export class ResultCenterComponent {
  render(data: ArtifactPage, append = false): void {
    const count = document.querySelector<HTMLElement>("#result-count");
    if (count) count.textContent = `${data.page.total} 項可追溯成果`;
    const target = document.querySelector<HTMLElement>("#result-catalog");
    if (!target) return;
    ...
    const more = document.querySelector<HTMLButtonElement>("#load-more-results");
    if (more) more.hidden = data.page.next_offset === null;
  }

  detail(item: ArtifactItem): string {
  ...
  private card(item: ArtifactItem): string {
    return `
  ...
  private thumbnail(item: ArtifactItem): string {
  ...
  private preview(item: ArtifactItem): string {
    const url = `/v1/workspace/results/${encodeURIComponent(item.artifact_id)}/preview`;
    if (item.artifact_type === "image") {
...
export class LibraryDomainComponent {
  render(data: LibraryDomainPage): void {
    const target = document.querySelector<HTMLElement>("#library-domains");
    if (!target) return;
  ...
  renderBook(book: Record<string, unknown>): string {
    const revisions = Array.isArray(book.revisions) ? book.revisions as Array<Record<string, unknown>> : [];
...
export const renderArchiveStatus = (data: ArchiveStatus): void => {
  const target = document.querySelector<HTMLElement>("#archive-summary");
  if (!target) return;
  target.innerHTML = [
