...
class MissionDevelopmentCouncil:
    ...
    _role_profiles = (
        {
            ...
            "mission": (
                "Name constraints and trade-offs, protect dependency direction, "
                "and prefer reversible designs."
            ),
            "required_outputs": [
                "bounded contexts",
                "alternatives",
        ...
        },
        {
            ...
            "mission": (
                "Deliver responsive, accessible, performant behavior rather than "
                "decorative markup."
            ),
            "required_outputs": [
                "responsive states",
        ...
        },
        {
            ...
            "required_outputs": ["severity", "evidence", "why it matters", "acceptance check"],
        ...
        },
        {
            ...
            "mission": (
                "Treat agent workflows as distributed systems with contracts, recovery, "
                "limits, and traces."
            ),
            "required_outputs": ["topology", "failure modes", "fallbacks", "termination condition"],
        ...
        },
        {
            "role": "ui-finish-gate",
            "mission": (
                "Reject generic interfaces unless product-specific hierarchy and "
                "responsive evidence are visible."
            ),
            "required_outputs": [
                "product lens",
                "finish decision",
            ...
            "source": "https://github.com/msitarzewski/agency-agents/blob/main/design/design-ui-finish-gate-reviewer.md",
            "license": "MIT",
        },
    )
    ...
    async def research_context(
        ...
        *,
        ...
        federated = self._federated_context(
            f"{mission.title} {mission.objective}",
        ...
        return {
            ...
            "role_profiles": self._profiles_for(project_type),
            "detail": detail,
            "source_policy": (
                        ...
                        "content": json.dumps(
                            {
                                ...
                                "review_profiles": self._profiles_for(project_type),
                                "files": files,
                            },
                            ensure_ascii=False,
                        ),
    ...
    def similar_patterns(
        ...
        *,
        ...
        for strategy in self._federated_context(
            f"{mission.title} {mission.objective}",
            project_type=project_type,
        )["strategies"]:
            pattern = strategy.get("learning_pattern")
            if not isinstance(pattern, dict):
                continue
            if pattern.get("project_type") not in {None, project_type}:
                continue
            tags = {str(item).casefold() for item in pattern.get("tags", [])}
            overlap = len(target_tokens & tags)
            ranked.append((1.5 + overlap / max(len(target_tokens), 1), pattern))
        ranked.sort(key=lambda item: item[0], reverse=True)
        return [pattern for _, pattern in ranked[:3]]

    def _federated_context(
        self,
        query: str,
        *,
        project_type: str,
        ...
        root = (self.settings.workspace_dir / "federation" / "installed").resolve()
        ...
        try:
            ...
            for item in items:
                ...
                text = json.dumps(capability, ensure_ascii=False).casefold()
                score = sum(token in text for token in tokens)
                ranked.append((score, item, path / "payload"))
            ranked.sort(key=lambda entry: entry[0], reverse=True)
            for _, item, payload_root in ranked[:6]:
                ...
                if pack_type == "knowledge_pack":
                    payload = json.loads(
                        (payload_root / "knowledge.json").read_text(encoding="utf-8")
                    )
                    result["knowledge"].extend(
                        {
                            "topic": run.get("topic"),
                            "conclusion_status": run.get("conclusion_status"),
                            "conclusion": run.get("conclusion"),
                            "confidence": run.get("confidence"),
                            "claims": run.get("claims", [])[:5],
                            "sources": run.get("sources", [])[:5],
                        }
                        for run in payload.get("runs", [])[:2]
                        if isinstance(run, dict)
                    )
                elif pack_type == "strategy_pack":
                    payload = json.loads(
                        (payload_root / "strategy.json").read_text(encoding="utf-8")
                    )
                    if payload.get("project_type") in {None, "general", project_type}:
                        result["strategies"].append(payload)
                elif pack_type == "evaluation_pack":
                    payload = json.loads(
                        (payload_root / "evaluation.json").read_text(encoding="utf-8")
                    )
                    result["evaluations"].extend(payload.get("runs", [])[:2])
                elif pack_type == "distillation_pack":
                    lines = (payload_root / "distillation.jsonl").read_text(
                        encoding="utf-8"
                    ).splitlines()
                    result["distillation_examples"].extend(
    ...
    @classmethod
    def _profiles_for(cls, project_type: str) -> list[dict[str, Any]]:
        roles = {"software-architect", "code-reviewer", "multi-agent-architect"}
        if project_type == "static_website":
            roles.update({"frontend-engineer", "ui-finish-gate"})
        return [dict(item) for item in cls._role_profiles if item["role"] in roles]
